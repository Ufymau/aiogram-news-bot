...
async def choose_news(callback: CallbackQuery) -> None:
    ...
    # Получаем язык пользователя из базы или другого хранилища
...
async def news_all(callback: CallbackQuery, lang_code: Optional[str] = None) -> None:
    """
        ...
        Retrieves all daily news for the user's language, translates and splits messages,
    ...
    news_messages: List[str] = get_all_daily(lang_code)
    translated_news: List[str] = translate_text(news_messages, lang_code)
    messages: List[str] = split_messages(translated_news)
...
async def process_keywords(message: Message, state: FSMContext) -> None:
    ...
    if news_messages:
        ...
        await message.answer(f"{NO_FRESH_NEWS.get(lang_code)}")
